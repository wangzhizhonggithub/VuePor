<template>
	<div class="tmpl">

		<!-- 面包屑 -->
		<el-row>
			<el-col :span="24">
				<div class="abread bt-line">
					<el-breadcrumb separator="/">
						<el-breadcrumb-item>购物商城</el-breadcrumb-item>
						<el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
						<el-breadcrumb-item>内容管理</el-breadcrumb-item>
					</el-breadcrumb>
				</div>
			</el-col>
		</el-row>

		<!--内容-->
		<el-row>
			<!--功能区-->
			<div class="goodInput">
				<el-row>
					<el-col :span="6">
						<el-button icon="plus">新增</el-button>
						<el-button icon="check">全选</el-button>
						<el-button icon="delete">删除</el-button>
					</el-col>

					<el-col :span="5" :offset="13">
						<el-input placeholder="请输入搜索内容" icon="search" v-model="input2" :on-icon-click="handleIconClick">
						</el-input>
					</el-col>

				</el-row>
			</div>
			<!--内容显示区-->
			<el-row>
				<el-col :span="24">
					<el-table ref="multipleTable" :data="list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
						<el-table-column type="selection" width="55">
						</el-table-column>
						<el-table-column label="标题" show-overflow-tooltip>
							<template scope="scope">{{ scope.row.title }}</template>
						</el-table-column>
						<el-table-column label="所属类别" width="150">
							<template scope="scope">{{ scope.row.categoryname }}</template>
						</el-table-column>
						<el-table-column label="库存" width="120">
							<template scope="scope">{{ scope.row.stock_quantity }}</template>
						</el-table-column>
						</el-table-column>
						<el-table-column label="市场价" width="120">
							<template scope="scope">{{ scope.row.market_price }}</template>
						</el-table-column>
						</el-table-column>
						<el-table-column label="销售价" width="120">
							<template scope="scope">{{ scope.row.sell_price }}</template>
						</el-table-column>
						</el-table-column>
						<el-table-column label="属性" width="150">
							<template scope="scope">0000</template>
						</el-table-column>
						</el-table-column>
						<el-table-column label="操作" width="88">
							<template scope="scope">修改</template>
						</el-table-column>
					</el-table>
				</el-col>
			</el-row>
			<!--翻页功能区-->
			<el-row>
				<el-col :span="12">
					<div class="pageTool">
						<el-pagination 
							@size-change="handleSizeChange" 
							@current-change="handleCurrentChange" 
							:current-page="pageSize" 
							:page-sizes="pageSizes" 
							:page-size="10" 
							layout="total, sizes, prev, pager, next, jumper" 
							:total="totalcount">
						</el-pagination>
					</div>
				</el-col>
			</el-row>

		</el-row>
	</div>
</template>

<script>
	export default {

		data() {

			return {
				input2: '',
				list: [],
				totalcount:'',
				pageSize:'',
				pageSizes:[],
        currentPage4: 1
			}

		},
		created() {
			//			调用获取数据的方法
			this.getList();
		},
		methods: {
			handleSelectionChange(val) {
				this.multipleSelection = val;
				console.log(123);
			},
			handleIconClick() {

			},
			 handleSizeChange(val) {
        console.log(`每页 ${val} 条`);
      },
      handleCurrentChange(val) {
        console.log(`当前页: ${val}`);
      },
			getList() {
				//				使用axios去发出具体的URL的请求获取到数据后捆绑到表格中
				var url = '/admin/goods/getlist?pageIndex=1&pageSize=10';
				this.$http.get(url).then(res => {
					if(res.data.status == 1) {
						this.$message.error(res.data.message);
						return;
					}
//					获取列表数据
					this.list = res.data.message;
//					获取总条数
					this.totalcount = res.data.totalcount;
//					获取页数
					this.pageSize = res.data.pageSize;
					var f = Math.ceil(this.totalcount/10);
					for(var i = 1;i<f+1;i++){
						this.pageSizes.push(10*i);		
					}
				})
			}
		}

	}
</script>
<style scoped>
	.abread {
		background: #fff;
		margin-bottom: 10px;
	}
	
	.goodInput {
		padding: 10px;
		border: 1px solid #f0f0f0;
		border-radius: 10px;
	}
	
	.pageTool{
		padding: 10px;;
	}
</style>